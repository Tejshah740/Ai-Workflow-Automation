...
async def get_user_by_email(conn: asyncpg.Connection, email: str):
    return await conn.fetchrow(
        "SELECT id, email, name, hashed_password, role, created_at FROM users WHERE email = $1",
        email,
    )
...
async def get_user_by_id(conn: asyncpg.Connection, user_id: int):
    return await conn.fetchrow(
        "SELECT id, email, name, hashed_password, role, created_at FROM users WHERE id = $1",
        user_id,
    )


async def create_user(conn: asyncpg.Connection, email: str, name: str, password: str):
